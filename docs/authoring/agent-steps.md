...
| `assert_no_side_effect: <kind>` | the run OBSERVED no side effect of the kind (`fs_write` or `http_request`); anywhere observation cannot run it fails "cannot certify" rather than passing vacuously. See [security.md](../agent-testing/security.md#side-effect-assertion-assert_no_side_effect) |
