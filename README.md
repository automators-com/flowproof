...
an HTML report, and by default a trace-synced `recording.gif` so a human can
watch what the run actually did. Use `--no-video`,
`--recording-detail low`, or `--recording-detail off` when speed matters more
than visual density. Add `--highlight-cursor` to render an unmistakable cursor
and click halo into the evidence frames. `flowproof heal` re-authors a broken flow
