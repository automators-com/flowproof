...
// How long the reader thread may block in one socket read. It holds the
// connection mutex for that whole read, and tungstenite's `read()` spans about
// two socket timeouts internally — so this constant really sets how long a CDP
// *send* can sit waiting for the lock, at roughly 2x its value.
//
// Profiling flowproof (78-character form flow, local fixture) found the reader
// holding the mutex 94% of wall-clock, sends waiting a mean of 10.7ms with this
// at 5ms, and the writes themselves costing 0.07ms. Upstream's 100ms made a
// single keystroke cost over 200ms.
//
// This is a mitigation, not the fix. The fix is to stop sharing one lock
// between reads and writes; until that lands this constant is the whole
// budget. Do not raise it, and do not "tidy" it into `idle_browser_timeout` —
// that is a different, much longer timeout with an unrelated job.
