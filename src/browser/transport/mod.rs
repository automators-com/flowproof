...
impl Transport {
        ...
        C: Method + serde::Serialize,
    {
        ...
        // Block on the response channel rather than polling it. The reader
        // thread delivers the instant a frame lands, so a poll that slept in
        // 5ms quanta charged up to 5ms of pure idle per call — and every
        // tab-level call paid it TWICE, once for the Target.sendMessageToTarget
        // ack and once for the real response. Measured on flowproof: the ack
        // wait fell from 5.25ms to 0.74ms.
        let response_result = response_rx
            .recv_timeout(self.idle_browser_timeout)
            .map_err(|_| util::Timeout);
